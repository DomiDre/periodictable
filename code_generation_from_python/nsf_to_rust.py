...
def to_uncertain(num:str) -> str:
    ...
    return insert_underscores(value)+"_f64" + ", " + insert_underscores(print_uncertainty)+"_f64"


elements_scattering_lengths = ""
isotope_scattering_lengths = ""
indentation = 12
current_atomic_number = 0
last_coherent_scatt_factor = 0.0

elements_scattering_lengths += " "*indentation + "scattering_lengths: [\n"


isotope_scattering_lengths += " "*indentation + "isotope_scattering_lengths: [\n"
isotope_scattering_lengths += " "*(indentation+4) + "vec![ // 0, neutron\n"
...
for line in nsftable.split("\n"):
    ...
    if atomic_number > current_atomic_number:
        isotope_scattering_lengths += " "*(indentation+4) + "],\n"
        isotope_scattering_lengths += " "*(indentation+4) + "vec![ // " + str(atomic_number) + ", "+ symbol + "\n"
    ...
    if len(mass_number) == 0:
        last_coherent_scatt_factor = b_c
        # is_element = True
    else:
    ...
    if abundance_or_half_life.endswith("Y") or abundance_or_half_life.endswith("S"):
        abundance = "0.0"
        half_time = float(abundance_or_half_life.split(" Y")[0])*365.25*24*3600 if abundance_or_half_life.endswith("Y") else float(abundance_or_half_life.split(" S")[0])
    else:
        abundance = abundance_or_half_life
        half_time = "0.0"
    
    # if abundance == "100" or\
    # atomic_number in [43, 61, 84, 85, 86, 87, 88, 89, 91, 93, 95]:
    #     is_element = True
    print_b_c = to_uncertain(b_c)
    print_bp = to_uncertain(bp)
    print_bm = to_uncertain(bm)
    is_plus_minus = "false"
    ...
    print_xs_coh = to_uncertain(xs_coh)
    print_xs_incoh = to_uncertain(xs_incoh)
    print_xs_total = to_uncertain(xs_total)
    print_xs_thermal_absorption = to_uncertain(xs_thermal_absorption)
    if not print_bp == "None":
        print_bp = "Some(UncertainFloat::new("+print_bp+"))"
    if not print_bm == "None":
        print_bm = "Some(UncertainFloat::new("+print_bm+"))"
    if not print_xs_coh == "None":
        print_xs_coh = "Some(UncertainFloat::new("+print_xs_coh+"))"
    if not print_xs_incoh == "None":
        print_xs_incoh = "Some(UncertainFloat::new("+print_xs_incoh+"))"
    if not print_xs_total == "None":
        print_xs_total = "Some(UncertainFloat::new("+print_xs_total+"))"
    if not print_xs_thermal_absorption == "None":
        print_xs_thermal_absorption = "UncertainFloat::new("+print_xs_thermal_absorption+")"

    if len(mass_number) != 0:
        if not print_b_c == "None":
            print_isotope_b_c = "Some(UncertainFloat::new("+ print_b_c +"))"
        else:
            print_isotope_b_c = print_b_c

        isotope_scattering_lengths += (
            " "*(indentation+8)+
            "IsotopeScatteringFactor::new(" + mass_number +
            ", " + str(float(abundance)) +
            ", " + str(float(half_time)) +
            ", " + str(float(eval(spin))) +
            ", " + print_isotope_b_c +
            ...
            ", " + print_bm +
            ", " + is_plus_minus +
            ", " + has_strong_energy_dependence +
            ", " + print_xs_coh +
            ", " + print_xs_incoh +
            ", " + print_xs_total +
            ", " + print_xs_thermal_absorption +
            # "UncertainFloat::new(" + to_uncertain(abundance) + ")"+
                "),\n"
        )
    if is_element:
        print_b_c = "UncertainFloat::new("+ print_b_c +")"
        elements_scattering_lengths += (
            " "*(indentation+4)+
            "ScatteringFactor::new(" +
            print_b_c +
            ", " + print_bp +
            ", " + print_bm +
            ", " + is_plus_minus +
            ...
            ", " + print_xs_thermal_absorption +
            # "UncertainFloat::new(" + to_uncertain(abundance) + ")"+
                "), // " + str(atomic_number) + ", "+ symbol + "\n"

        )

elements_scattering_lengths += " "*indentation + "],\n"
isotope_scattering_lengths += " "*(indentation+4) + "]\n"
isotope_scattering_lengths += " "*indentation + "]"

with open("rust_nsf.rs", "w") as f:
    f.write(elements_scattering_lengths)
    f.write(isotope_scattering_lengths)